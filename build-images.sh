...
# Reuse existing nodebuilder-planka container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-planka; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-planka -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=ghcr.io/plankanban/planka:2.1.1 docker.io/postgres:16.14-alpine" \
    "${container}"
# Commit the image
